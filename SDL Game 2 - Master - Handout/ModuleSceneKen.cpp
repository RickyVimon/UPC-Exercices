...
ModuleSceneKen::ModuleSceneKen(bool start_enabled) : Module(start_enabled)
{
	...
	foreground.frames.push_back({ 8, 23, 521, 194 });
	foreground.frames.push_back({ 8, 24, 521, 194 });
	foreground.frames.push_back({ 8, 25, 521, 194 });
	foreground.frames.push_back({ 8, 26, 521, 194 });
	foreground.frames.push_back({ 8, 27, 521, 194 });
	foreground.frames.push_back({ 8, 28, 521, 194 });
	foreground.frames.push_back({ 8, 28, 521, 194 });
	foreground.frames.push_back({ 8, 27, 521, 194 });
	foreground.frames.push_back({ 8, 26, 521, 194 });
	foreground.frames.push_back({ 8, 25, 521, 194 });
...
bool ModuleSceneKen::Start()
{
	...
	App->audio->PlayMusic("ken.ogg", 4.0f);
...
update_status ModuleSceneKen::Update()
{
	...
	App->renderer->Blit(graphics, 0, 0, &(foreground.GetCurrentFrame()), 1.1f);

	// TODO 6: Draw the girl. Make sure it follows the ship movement!	
	App->renderer->Blit(graphics, 192, 128 - foreground.GetCurrentFrame().y, &(girl.GetCurrentFrame()), 1.1f);
